...
process_chunk() {
  ...
  mkdir -p csv_parallel/${chunk_id}/imm_pomdp_softmax_temp_0.5
  for ds in ${dss[@]}; do
    for seed in $(seq $start $end); do
      python main.py --epochs $ds --batch_size 50 --gamma 0.9 --seed $seed --lambda_ratio -1 --pomdp_temp 0.5 --output_dir csv_parallel/${chunk_id}/imm_pomdp_softmax_temp_0.5 > /dev/null 2>&1
    done
  done
}
...
mkdir -p csv/imm_pomdp_softmax_temp_0.5
for ds in ${dss[@]}; do
  for ((i = 0; i < PARALLEL_JOBS; i++)); do
    partial_file_path="csv_parallel/${i}/imm_pomdp_softmax_temp_0.5/epochs=$ds,lambda_ratio=*.csv"
    partial_file_name=$(basename $partial_file_path)
    cat $partial_file_path >> "csv/imm_pomdp_softmax_temp_0.5/$partial_file_name"
  done
done
